...
def find_and_warp(frame, source, cornerIds, dictionary, parameters): 
    (imgH, imgW) = frame.shape[:2] #grab  the width and height of frame and source image
    ...
    if len(markerCorners)!= 4:
        ...
        markerIds.flatten()
    refPts = []  #global variable initaialize  our list of reference point
    for i in cornerIds:  #then we could not find the marker with the current ID
        ...
        #print(markerCorners)
        corner = np.squeeze(markerCorners[j]) # of reference points
